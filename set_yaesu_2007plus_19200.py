#!/usr/bin/env python3
"""LIVE COMMIT: navigate CAT -> YAESU -> "FTxxxx 2007+" -> baud page and
ensure 19200 is selected before confirming. Display-verified at every step;
aborts via DISPLAY (no programming effect) on any mismatch. Re-enters the
baud page afterward (read-only, cancels) to confirm the model + baud stuck.
"""
import os
import sys
...
def enter_baud_page():
    ...
    on_model = navigate_to(grid, TARGET_MODEL_SUBSTR, 18)
    hi = highlighted_text(on_model)
    if "band-data" in hi.lower():
        bail(f"navigation landed on BAND-DATA instead of {TARGET_MODEL_SUBSTR}, aborting")

    press("set")
    baud_page = wait_for_change(on_model, tries=8, delay=1.0)
    if baud_page is None:
        bail(f"no change entering {TARGET_MODEL_SUBSTR} baud page")
...
def main():
    print("=== Step 1: enter baud page, check current value ===")
    splash, baud_page = enter_baud_page()
    _, hi_before = show("FTxxxx 2007+ baud page (before)", baud_page)
    print(f"\ncurrent baud: {hi_before!r}")

    cur = baud_page
    if hi_before.upper() != TARGET_BAUD:
        print(f"\n=== navigating to {TARGET_BAUD} ===")
        cur = navigate_to(baud_page, TARGET_BAUD, 8)
        _, hi_check = show("cursor before commit", cur)
        if hi_check.upper() != TARGET_BAUD:
            bail(f"expected {TARGET_BAUD} highlighted before commit, got {hi_check!r}")
    else:
        print(f"\nAlready {TARGET_BAUD} -- confirming as-is.")

    print(f"\n=== Step 2: COMMITTING -- pressing SET on {TARGET_BAUD} (model=FTxxxx 2007+) ===")
    press("set")
    committed = wait_for_change(cur, tries=8, delay=1.0)
    ...
    print("\n=== Step 3: re-enter baud page read-only to verify it stuck ===")
    splash2, baud_page2 = enter_baud_page()
    _, hi_verify = show("FTxxxx 2007+ baud page (verify)", baud_page2)
    cancel_to_splash(baud_page2, splash2)

    if hi_verify.upper() == TARGET_BAUD:
        print(f"\nCONFIRMED: FTxxxx 2007+ baud is {TARGET_BAUD}.")
    else:
        print(f"\nWARNING: expected {TARGET_BAUD} on verify, got {hi_verify!r} -- check front panel.")

    st = get_status()
    print(f"\nfinal status: operatingState={st.get('operatingState')} catInterface={st.get('catInterface')} bandText={st.get('bandText')} warningCode={st.get('warningCode')}")
